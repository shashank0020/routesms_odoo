...
class report_print_check(report_sxw.rml_parse):
    ...
    def get_partner(self,partner):
        ...
        if partner :
            ...
            exec('''map(lambda partner:partner_address.append(partner.name) if partner.name else None,[partner])\nmap(lambda partner:partner_address.append(partner.street) if partner.street else None,[partner])\nmap(lambda partner: partner_address.append(partner.street2) if partner.street2 else None,[partner])\nmap(lambda partner:partner_address.append(partner.city) if partner.city else None,[partner])\nmap(lambda partner:partner_address.append(partner.state_id.name)  if  partner.state_id else None,[partner])\nmap(lambda partner:partner_address.append(partner.country_id.name) if partner.country_id  else None,[partner])\nmap(lambda partner:partner_address.append(partner.zip) if partner.zip  else None,[partner])\n''')

            
            if partner_address :
                vals=','.join(partner_address)
...
class report_check(osv.AbstractModel):
    _name = 'report.routesms.report_check'
    _inherit = 'report.abstract_report'
    _template = 'routesms.report_check'
