...
class account_asset_category(models.Model):
    ...
    parent_id= fields8.Many2one('account.asset.category',string='Parent Category')
    

class account_asset_asset(models.Model):
    _inherit = 'account.asset.asset'
    
    employee_id= fields8.Many2one('hr.employee',string='Employee')
...
class account_tax(osv.osv):
    ...
    _columns={
              'tax_name_report': fields.char('Tax on report',size=16),
              'compute_child_on_base':fields.boolean('Compute Child On Base Amount'),
              
              }   

    _defaults={
               'compute_child_on_base':False,
               }   
...
class account_move(osv.osv):
    ...
    def onchange_jv_period(self,cr,uid,ids,period_id,context):
        ''' Check if period selected belongs to same company'''
        res={}
        if period_id : 
            if self.pool.get('account.period').browse(cr,uid,period_id).company_id.id !=self.default_get(cr,uid,['company_id']).get('company_id') :
                raise osv.except_osv(_('Warning !'), _("Period Belongs To Other Company\nSelect Valid Period & Continue"))
        return res
        
    
    def onchange_jv_date(self,cr,uid,ids,date,context):
        ''' Make date and period equal'''
        ...
        if date :
            try : 
                if context.get('company_id') : 
                    ...
                    company=self.default_get(cr,uid,['company_id']).get('company_id')
                
                if self.default_get(cr,uid,['company_id']) : 
                    company=self.default_get(cr,uid,['company_id']).get('company_id')
                    period_dates=[ period.id for period in period_obj.browse(cr,uid,period_obj.search(cr,uid,[('company_id','=',company )]) ) \
                                  if 'opening' not in period.name.lower() and  '-'.join(period.date_start.split('-')[:2])=='-'.join(date.split('-')[:2]) ]
                    
                    if len(period_dates) > 1 or not period_dates : 
                        raise osv.except_osv(_('Error!'), _("No Period Found For Fiscal Year"))
                    #return {'value':{'period_id':period_dates[0]}}
                    default_period_id=self.default_get(cr,uid,['period_id']).get('period_id')
                    
                    if default_period_id !=period_dates[0] : 
                        raise osv.except_osv(_('Warning !'), _("Make Sure You Have Selected Correct Period"))
                    res.update({'value':{'period_id':period_dates[0],'to_check':True} } )
                    return res
                
            except Exception as E : 
                raise osv.except_osv(_('Error!'), _('{}').format(str(E.value)))
            
        raise osv.except_osv(_('Error!'), _("Unable To Update Document/nContact Odoo Team "))
    ...
    def create(self,cr,uid,vals,context=None):
        ...
        #import ipdb;ipdb.set_trace()
        if self.browse(cr,uid,move_id).state=='draft': 
            #subject='DRAFT',
            self.message_post(cr, uid, [move_id], body=_("Unposted"), context=context)
        else : 
            
            self.message_post(cr, uid, [move_id], body=_("Posted"), context=context)
        return move_id
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        self.message_post(cr, uid, ids, body=_("Document Updated"), context=context)
        #
#         if self.browse(cr,uid,ids[0]).state=='draft': 
#             subject='Document Updated',
#             self.message_post(cr, uid, ids,subject=_(subject), body=_("Unposted"), context=context)
#         else : 
#             subject='POST',
#             self.message_post(cr, uid, ids,subject=_(subject), body=_("Posted"), context=context)
        return res     
    ...
    def button_validate(self, cursor, user, ids, context=None):
        ...
        self.message_post(cursor, user, ids, body=_("Posted"), context=context)
    ...
    def button_cancel(self, cr, uid, ids, context=None):
        ...
        ##pass message posting if len of JV is >1 
        if len(ids)== 1 :
            self.message_post(cr, uid, ids, body=_("Unposted"), context=context)
...
class account_invoice(osv.osv):
    ...
    def _period_to_words(self,cr, uid, ids, name, args, context=None):
        ...
        cr.execute('''select name_convention from res_company ''')
        conventions=map(lambda x:x[0],cr.fetchall())
        ...
        for invoice in self.browse(cr, uid, ids) :
            ...
            if period : 
                ...
                #check_convention=[x for x in period.split(' ') if x in conventions]
                check_convention=[x for x in conventions if x in period.split(' ')]
                
                if check_convention : 
                    period=period.replace(check_convention[0],'').strip()
                    split=period.split('/')
                else : 
    ...
    def _count_total_invoice_line(self,cr, uid, ids, name, args, context=None):
       
        ''' store total  no of invoice lines for reporting purpose'''
        ...
        for invoice in self.browse(cr, uid, ids) :
            ...
            if invoice.invoice_line : 
                ...
                res[invoice.id]=len(invoice.invoice_line)
...
#             res[invoice.id]=','.join(ticket_nos)
    ...
    _columns = {
        ...
        'product_id': fields.related('invoice_line', 'product_id', type="many2one", relation="product.product", string="Product",store=True ),        
        'total_invoice_line':fields.function(_count_total_invoice_line,string='Total Invoice Lines', type='integer',store=True),
    ...
    def invoice_validate_old_28jan(self,cr,uid,ids):
        ''' Method overided to restrict invoices created for RSL group'''
        
        ##check if partner is type company
        if uid==SUPERID : 
            raise osv.except_osv(_('Error!'),_("SuperUser Cannot Validate Invoice Due To Security Reasons!"))
            
        ##update the responible user of invoice document##
        self.write(cr,uid,ids,{'responsible':uid})
        
        
        current_invoice_record=self.browse(cr,uid,ids[0])
        if current_invoice_record.partner_id : 
            if current_invoice_record.partner_id.is_company==False : 
                raise osv.except_osv(_('Error!'),_("Partner is not a company !"))
       
        if current_invoice_record.date_invoice :
            pass
        
        else :
            raise osv.except_osv(_('Error!'),_("Invoice Date is required !"))
        
        if current_invoice_record.responsible.id==SUPERID or current_invoice_record.employee_id.id==SUPERID  : 
            raise osv.except_osv(_('Error!'),_("Invoice Document Contains SuperUser!"))
    ...
    def check_company_period(self,cr,uid,invoice):
        ''' Check if period selected belongs to same company'''
        res={}
        
        if invoice.period_id : 
            if invoice.period_id.company_id.id !=invoice.company_id.id :
                raise osv.except_osv(_('Warning !'), _("Period Belongs To Other Company\nSelect Valid Period & Continue"))
        return res
    
    def check_date_period(self,cr,uid,invoice):
        ''' Check date and period equal'''
        ...
        if invoice.date_invoice :
            ...
            try : 
                ...
                period_dates=[ period.id for period in period_obj.browse(cr,uid,period_obj.search(cr,uid,[('company_id','=',invoice.company_id.id )]) ) \
                              if 'opening' not in period.name.lower() and  '-'.join(period.date_start.split('-')[:2])=='-'.join(date.split('-')[:2]) ]
                
                if len(period_dates) > 1 or not period_dates : 
                    raise osv.except_osv(_('Error!'), _("No Period Found For Fiscal Year"))
                
                if invoice.period_id.id !=period_dates[0] : 
                    raise osv.except_osv(_('Warning !'), _("Make Sure You Have Selected Correct Period"))
    ...
    def invoice_validate(self,cr,uid,ids):
        ...
        if current_invoice_record.company_id.id== 3 :
            return self.write(cr,uid,ids,{'state': 'open'})

        ##check if period and date are same###
        self.check_date_period(cr,uid,current_invoice_record)
        
        ##check if period belongs to same company##
        self.check_company_period(cr,uid,current_invoice_record)

        vals={'currency_id':current_invoice_record.currency_id.id,'date_invoice':current_invoice_record.date_invoice}
        currency_status=self.currency_set(cr,uid,vals,{})

        if currency_status :

            if current_invoice_record.company_id.id== 1 :
                raise osv.except_osv(_('Error!'), _('Invoice cannot be validated for RSL (Group) Company.'))
            if current_invoice_record.partner_bank_id :
