...
#####GET BM IDS
        ...
        cr.execute(''' select * from res_groups_users_rel where gid in %s and uid=%s''',(tuple([21,22,23]),uid))
        bm_invoices_view=map(lambda x:x[0],cr.fetchall())
        ...
        for rule in self.browse(cr, uid, ids, context):
            ...
            if uid in [295,454,521,435,327,321,318] and rule.id==144 and self.pool.get('res.users').browse(cr,uid,uid).company_id.id==1: #krupali
                ...
                if rule.domain_force:
                    ...
                    res[rule.id] = expression.normalize_domain(eval(custom_rule, eval_context))
                else:
                    res[rule.id] = []                     
                 
            
            elif not bm_invoices_view and rule.id==144 :
                 
                ###see my invoices for BM's 
                custom_rule="['&',('user_id','='," + str(uid) +"),('company_id','=',"+str(self.pool.get('res.users').browse(cr,uid,uid).company_id.id)+")]"
                res[rule.id] = expression.normalize_domain(eval(custom_rule, eval_context)) 
                         
            elif uid in [466,384,579,373,454] : #indian accounting rights
            #elif uid in [466,384,579] :
                 
                 
                if rule.id==147  : #partner rule and Hemkant Wakchoure
                    
                    if rule.domain_force:
                        custom_rule="[('country_id','in',[105,255])]"
                        print '-----------RULE FOR Hemkant Wakchoure-----------'
                        res[rule.id] = expression.normalize_domain(eval(custom_rule, eval_context))
                    else:
                        res[rule.id] = []     
                    ...
                    if uid in [384]  and self.pool.get('res.users').browse(cr,uid,uid).company_id.id==1: #for RSL group rights to migrate invoices to other companies
                        cr.execute('''select id from res_partner where country_id in (105,255)''') 
                        
                        domain_cus="[('partner_id','in',"+ str(map(lambda x:x[0],cr.fetchall())) +"),('company_id','in',"+str(company_obj.search(cr,uid,[]))+")]"
                ...
                elif rule.id==39 or rule.id==49 :
                    
                    
                    
                    cr.execute('''select id from res_partner where country_id in (105,255)''')
                    partner_ids=str(map(lambda x:x[0],cr.fetchall()))
                    
                    if uid in [384]  and self.pool.get('res.users').browse(cr,uid,uid).company_id.id==1: #for RSL group rights to migrate invoices to other companies
                        
                        domain_cus="[('partner_id','in',"+partner_ids +"),('company_id','in',"+str(company_obj.search(cr,uid,[]))+")]"
                    
                    else : 
                        
                        domain_cus="[('partner_id','in',"+ partner_ids +"),('company_id','=',"+str(self.pool.get('res.users').browse(cr,uid,uid).company_id.id)+")]"
                         
                    res[rule.id] = expression.normalize_domain(eval(domain_cus, eval_context))       
                         
                     
#                 elif rule.id==29 :       
#  
#                     domain_cus="[('journal_id.currency','=',"+ str(False) +"),('company_id','=',"+str(self.pool.get('res.users').browse(cr,uid,uid).company_id.id)+")]"
#                      
#                     res[rule.id] = expression.normalize_domain(eval(domain_cus, eval_context))                      
            
                else:
                    print '-----------RULE for others-----------'   
                    if rule.domain_force:
                        res[rule.id] = expression.normalize_domain(eval(rule.domain_force, eval_context))
                    else:
                        res[rule.id] = []
