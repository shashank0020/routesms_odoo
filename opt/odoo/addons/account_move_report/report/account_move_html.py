...
class account_move_report_html(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'get_responsible_person':self.get_responsible_person,
        })
    ...
    def get_responsible_person(self,obj): 
        ...
        for move in obj : 
            cr.execute('''select write_uid from account_move where id=%s ''',(move.id,))
            user_id=map(lambda x:x[0],cr.fetchall())
            if user_id : 
                user_name=self.pool.get('res.users').read(cr,SUPERUSER,user_id[0],['name'])
                
        return user_name.get('name','No User Found') if user_name else ''
