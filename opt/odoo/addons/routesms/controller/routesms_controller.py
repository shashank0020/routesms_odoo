...
class QMS_controller(openerp.http.Controller):
   # @route('/<int>/', auth='public')

    @route("/update_partner_status/<username>/<servername>/<odooid>", auth='none')
    def update_partner_account_status(self, username, servername, odooid): 
	#import ipdb;ipdb.set_trace()
        '''API to set partner status from Test To Live '''
        request.uid=SUPERID
        api_obj = request.env['routesms.api']
        vals={'username' : username, 'servername' : servername, 'odooid' : odooid}
        user_response=api_obj.update_partner_account_status(vals)
        return user_response 

    @route("/validate_query/<email>/<odooid>/", auth='public')
    def qms_validate_uery(self,email,odooid):
        
        api_obj = request.env['routesms.api']

        vals={'email':email,'odooid':odooid}

        qms_response=api_obj.qms_validate_query_integration(vals)

        return qms_response


    @route("/swap/<email>/<odooid>/", auth='public')
    def qms_swap_uery(self,email,odooid):
	
#        import ipdb;ipdb.set_trace()
        api_obj = request.env['routesms.api']
        #cr, uid, context = request.cr, request.uid, request.context
        vals={'email':email,'odooid':odooid}
        qms_response=api_obj.qms_swap_integration(vals)
...
#        uid=1
        ...
        cr.execute(''' select * from  res_groups_users_rel where gid in (21,22,23) and uid =%s''',(uid,))
